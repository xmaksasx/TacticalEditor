    ...
    class CalculateAirBases
    {
        ...
        private void AircraftCoordinateChange(AircraftPosition aircraft)
        {
            for (int i = 0; i < _airBasePoints?.Length; i++)
            {
                ...
                _coordinateHelper.LocalCordToXZ(abp.GeoCoordinate.Latitude, abp.GeoCoordinate.Longitude, abp.GeoCoordinate.Latitude, abp.GeoCoordinate.Longitude,
                    out var x, out var z);
                _airBasePoints[i].AirportInfo.Runway.Threshold.X = x - acG.X;
                _airBasePoints[i].AirportInfo.Runway.Threshold.Z = z - acG.Z;

                _coordinateHelper.LocalCordToXZ(abpR.Threshold.Latitude, abpR.Threshold.Longitude, abpR.GlideSlope.Latitude, abpR.GlideSlope.Longitude,
                    out  x, out  z);
                _airBasePoints[i].AirportInfo.Runway.GlideSlope.X = x - acG.X;
                _airBasePoints[i].AirportInfo.Runway.GlideSlope.Z = z - acG.Z;

                _coordinateHelper.LocalCordToXZ(abpR.Threshold.Latitude, abpR.Threshold.Longitude, abpR.Localizer.Latitude, abpR.Localizer.Longitude,
                    out x, out z);
                _airBasePoints[i].AirportInfo.Runway.Localizer.X = x - acG.X;
                _airBasePoints[i].AirportInfo.Runway.Localizer.Z = z - acG.Z;

                _coordinateHelper.LocalCordToXZ(abpR.Threshold.Latitude, abpR.Threshold.Longitude, abpR.LocatorMiddle.Latitude, abpR.LocatorMiddle.Longitude,
                    out x, out z);
                _airBasePoints[i].AirportInfo.Runway.LocatorMiddle.X = x - acG.X;
                _airBasePoints[i].AirportInfo.Runway.LocatorMiddle.Z = z - acG.Z;

                _coordinateHelper.LocalCordToXZ(abpR.Threshold.Latitude, abpR.Threshold.Longitude, abpR.LocatorOuter.Latitude, abpR.LocatorOuter.Longitude,
                    out x, out z);
